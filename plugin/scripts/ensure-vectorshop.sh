...
# 必须在下面这行覆盖同名变量之前,先把调用方(若有)传入的 PLUGIN_ROOT 另存一份:
# `PLUGIN_ROOT` 这个名字同时也是 Codex hooks 引擎塞进 hook 命令环境的变量(见
# migrate_legacy 的第二判据),而本文件早就用同一个名字表达「脚本自身所在目录」——
# 两个含义撞在一起。如果不抢先另存,下面这行会把调用方传入的值整个覆盖掉,
# migrate_legacy 里读到的 PLUGIN_ROOT 就永远是脚本自己算出来的路径(必然非空),
# 把「调用方是不是 Codex hook」这个判据变成永远为真——曾经这样把 Claude 路径也一起
# 短路掉,被 scripts/tests/test_plugin.sh 的既有 migrate: 系列用例抓到过。
VECTORSHOP_CALLER_PLUGIN_ROOT="${PLUGIN_ROOT:-}"
...
migrate_legacy() {
  ...
  # 读的是文件顶部抢先另存的 VECTORSHOP_CALLER_PLUGIN_ROOT,不是裸 PLUGIN_ROOT——本
